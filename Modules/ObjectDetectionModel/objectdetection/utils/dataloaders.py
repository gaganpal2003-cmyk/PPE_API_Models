...
class LoadImagesOCR:
    ...
    def __init__(self, path_or_img, img_size=640, stride=30, auto=True, transforms=None, vid_stride=1):
        ...
        if isinstance(path_or_img, np.ndarray):
            ...
            if isinstance(path_or_img, str) and Path(path_or_img).suffix == ".txt":
                path_or_img = Path(path_or_img).read_text().rsplit()[0]
            im0 = cv2.imread(path_or_img)
            assert im0 is not None, f"Image Not Found {path_or_img}"
...
class LoadImages:
    # YOLOv5 image/video dataloader, i.e. `python detect.py --source image.jpg/vid.mp4`
    def __init__(self, path, img_size=640, stride=30, auto=True, transforms=None, vid_stride=1):
        if isinstance(path, str) and Path(path).suffix == ".txt":  # *.txt file with img/vid/dir on each line
            path = Path(path).read_text().rsplit()
        # files = ['C:\\Users\\Ramesh\\OneDrive\\Desktop\\GUI\\Work\\Yolo\\Version_05\\Modules\\ObjectDetectionModel\\objectdetection\\video_01.mp4']
        files = [path]
    ...
    def _new_video(self, path):
        ...
        self.frame = 0
        self.count = 0
        self.cap = cv2.VideoCapture(path, cv2.CAP_ANY, [cv2.CAP_PROP_HW_ACCELERATION, cv2.VIDEO_ACCELERATION_ANY])
        self.frames = int(self.cap.get(cv2.CAP_PROP_FRAME_COUNT) / self.vid_stride)
        self.orientation = int(self.cap.get(cv2.CAP_PROP_ORIENTATION_META))  # rotation degrees
        # self.cap.set(cv2.CAP_PROP_ORIENTATION_AUTO, 0)  # disable https://github.com/ultralytics/yolov5/issues/8493
        print("call _new_video")
